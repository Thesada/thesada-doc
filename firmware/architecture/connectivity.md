...
flowchart LR
    ...
    B --> C{force?}
    C -->|No| D{Newer version?}
    D -->|No| E[Skip]
    D -->|Yes| F[Download .bin]
    C -->|Yes| F
    F --> G{SHA256 match?}
    G -->|No| H[Abort]
    G -->|Yes| I[Flash]
...
### Legacy cmd/* topics (replaced in v1.2.3)

The old hardcoded `cmd/config/set` and `cmd/config/push` topics are gone. Use the CLI equivalents:
...
The `<prefix>/cmd/ota` topic is still supported as an optional dedicated trigger when `ota.cmd_topic` is set in `config.json`. For most use the `cli/ota.check` path above - it calls the same code path, accepts the `--force` flag, and its response is visible on `<prefix>/cli/response`, so failures are diagnosable instead of silent.
